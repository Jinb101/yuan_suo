<template>
  <v-view header @on-user="onUser">
    ...
    <div class="emoji_class_box">
      <div id="emoji-class" class="class_input"></div>
      <div class="class_upload flex fw">
        <template v-if="+radio === 1">
          <van-cell title="添加图片" />
          <!-- <van-cell title="添加图片" label="最大限制30张" /> -->
        ...
        <template v-else>
          <van-cell title="添加视频" label="横屏拍摄效果佳!视频最长30秒" />
          <v-upload-once
            class="pic2"
            type="video"
            v-model="vio"
    ...
    <div class="lines" v-if="is_change_class">
    ...
    <van-cell title="推送给老师" icon="friends-o"></van-cell>
    <v-p api v-model="allPeople"></v-p>
    <div class="emoji_box">
      <div class="emoji_box_line">
        <span class="emoji_icon" @click.stop="onEmoji">
          <i class="van-icon van-icon-smile-o" v-if="!open"></i>
          <i class="van-icon van-icon-ascending" v-else></i>
        </span>
      </div>
      <div class="emoji_change demo_scrollbar" v-if="open">
        <div class="emoji_line" v-for="(i, j) in icon" :key="j">
          <h5>{{ i.text }}</h5>
          <ul class="flex fw">
            <li
              v-for="x in i.count"
    ...
    <template #fixed>
      <v-f :footer="false" v-model="class_open">
        <van-radio-group v-model="class_check">
          <van-cell-group>
            <van-cell title="全部班级" clickable @click="class_check = '1'">
              <template #right-icon><van-radio name="1" /></template>
            </van-cell>
            <van-cell title="部分班级" clickable @click="class_check = '2'">
            ...
            <template v-if="class_check === '2'">
              <div class="select_class">
                <van-button
                  type="info"
                  size="small"
                  @click="$refs.checkboxs.toggleAll(true)"
                  >全选</van-button
                >
                <van-checkbox-group v-model="class_checkbox" ref="checkboxs">
                  <van-cell-group>
                    <van-cell
                      :title="x.nickname"
                      clickable
            ...
            <van-cell title="当前班级" clickable @click="class_check = '3'">
              <template #right-icon><van-radio name="3" /></template>
            </van-cell>
          </van-cell-group>
        </van-radio-group>
        <div class="end_btn">
          <van-button block round type="primary" @click="class_open = false"
            >确认选中</van-button
          >
        </div>
      </v-f>
...
export default {
  ...
  mounted() {
    ...
    tinymce.init({
      ...
      auto_focus: true,
      // plugins:
      //   "preview searchreplace autolink fullscreen image axupimgs link code codesample table charmap hr advlist lists textpattern emoticons autosave indent2em formatpainter",
      // toolbar: [
      //   "image axupimgs indent2em code preview undo redo restoredraft",
      //   "bold italic underline strikethrough link",
      //   "alignleft aligncenter alignright",
      //   "table charmap emoticons hr",
      //   "cut copy forecolor backcolor",
      ...
      //       (res) => {
      //         that.$api.upload({ imgs: res.file, type: "img" }, (rej) => {
      //           callback(rej.data);
      //         });
      //       }
      //     );
