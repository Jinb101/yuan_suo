        ...
        <div class="searchs">
          <div class="search_job" v-if="+i.show_search === 1">
            <ul>
              ...
              <li class="none" v-if="i.ify" @click.stop="onAddOnceJob(j)">
                添加一项
              </li>
            </ul>
          </div>
...
export default {
  ...
  methods: {
    ...
    onAddOnceJob(j) {
      this.select_index = j;
      this.showAdd = true;
      this.showAddExt = this.list[j];
      this.$set(this.list[j], "jid", "");
    },
    toJob(n, index) {
      if (!n.id) return 0;
      this.$set(this.list[index], "job", n.name || n.text);
