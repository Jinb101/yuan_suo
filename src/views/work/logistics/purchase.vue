  ...
  <v-view header cover @on-dot="ondot">
    ...
    <div class="tab">
      <van-tabs v-model="type" title-active-color="#38f" @click="onPage(1)">
        <van-tab title="待入库" name="1" :dot="dot > 0"></van-tab>
        <van-tab title="已入库" name="2"></van-tab>
    ...
    <template #fixed>
      <v-f :footer="false" v-model="open" :text="det.name">
        ...
        ></van-cell>
        <template v-if="type === '1'">
          <p class="msg">状态为待入库时，可对物品单价/总价进行更改</p>
          <div class="w80 flex">
            <van-button block round type="info" @click="onRevote"
              >撤销</van-button
            >
...
export default {
  ...
  watch: {
    ...
    type(n) {
      if (n > 0) {
        this.appPath("/work_purchase/" + n, true);
      } else {
        this.appPath("/work_purchase", true);
      }
    },
  },
  methods: {
    ...
    onRevote() {
      this.$js.model("撤销提示", "是否撤销？", (r) => {
        if (r) {
          let purchase_id = this.det.purchase_id;
          this.$api.http("purchasedel", { purchase_id }, () => {
            this.$toast("撤销成功");
            this.dot -= 1;
            this.open = false;
            this.isevent = true;
    ...
    onEnter() {
      this.$js.model("入库确认", "是否确认入库,入库后单价不可更改", (r) => {
        if (r) {
          let purchase_id = this.det.purchase_id;
          this.$api.http(
            "purchasesub",
            { purchase_id, price: this.price, total_price: this.amount },
            () => {
              this.$toast("入库成功");
              this.dot -= 1;
              this.open = false;
