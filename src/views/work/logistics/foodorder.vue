        ...
        <div class="foodorder">
          <div class="box_line">
            <template v-if="goods.length">
              ...
              <ul>
                <li v-for="(x, y) in goods"
                    :key="y + 'g'"
                    class="flex">
                  <span>{{ x.goods_name }}</span>
                  <template v-if="+statusid === 1">
                    <div class="q">
                      <van-field v-model="x.number"
                                 type="digit"
                                 readonly
                                 input-align="center" />
                    </div>
                    <div class="q qs">
                      <van-field v-model="x.num"
                                 type="digit"
                                 placeholder="请输入"
                                 :readonly="+x.status === 2"
                                 input-align="center">
          ...
          <div class="cont_view">
            ...
            <p>
              联系人电话：<b class="col_primary"
                 @click.stop="ontel(det.contact_number)">{{ det.contact_number }}</b>
            </p>
          ...
          <div class="tips">
            ...
            <div class="tip_li"
              ...
              所属分类为<span>{{ q.class_name }}</span>单价<span>{{ +q.price }}元/{{ q.unit }}</span>
              <van-tag type="danger"
                       round
                       text-color="#fff"
                       v-if="q.status === 2">缺货</van-tag>
