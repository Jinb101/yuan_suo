  ...
  <v-view header bgc="#f5f5f5">
    ...
    <v-page ref="page" @on-page="onPage">
      ...
      >
        ...
        <div class="logo">
          <v-a teacher :sex="i.gender" :src="i.avatar"></v-a>
        </div>
        <div class="text">
          <h6>{{ i.name }}</h6>
          <p>手机号：{{ i.contact }}</p>
    ...
    <template #fixed>
      <v-t v-model="os.start_time" :open="open1" max="d_5" ymd></v-t>
      <v-t v-model="os.end_time" :open="open2" max="d_15" ymd></v-t>
      <v-f v-model="open3" :footer="false" text="老师详情">
        <van-cell title="姓名：" :value="detail.name"></van-cell>
        <van-cell title="昵称：" :value="detail.nickname"></van-cell>
        <van-cell
          title="手机号："
        ...
        <van-cell
          ...
          :value="detail.phone"
          is-link
          @click="ontel(detail.phone)"
        ></van-cell>
        <van-cell title="账号：" :value="detail.account"></van-cell>
        <van-cell title="部门：" :value="detail.group_name"></van-cell>
        <van-cell title="岗位：" :value="detail.job_name"></van-cell>
        ...
        <van-cell
          title="审批时间："
          :value="appTimeout(detail.approval_time)"
        ></van-cell>
        <van-cell title="备注：" :label="detail.remarks || '无'"></van-cell>
        <div class="w80 flex">
          <van-button
            round
            type="danger"
            block
            @click.stop="onevent('del', detail)"
            >删除</van-button
          >
          <template v-if="+detail.examine === 2">
            ...
            <van-button
              ...
              type="info"
              block
              @click.stop="onevent('ok', detail)"
              >通过</van-button
            >
          </template>
        </div>
      </v-f>
      <v-f v-model="open4" :footer="false" text="驳回申请" bgc="#f5f5f5">
        <van-field
          v-model="brief"
          rows="3"
          autosize
