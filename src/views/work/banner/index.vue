<template>
  <v-view header bgc="#f7f7f7">
    ...
    <v-page ref="page" @on-page="onPage">
      ...
      >
        <div class="logo" @click.stop="appShowImage(i.picture)">
          <van-image :src="i.picture" lazy-load fit="cover"></van-image>
        </div>
        <div class="f1">
          <h6 class="van-ellipsis">{{ i.describe }}</h6>
    ...
    <template #fixed>
      <v-f :footer="false" v-model="open" text="编辑轮播图" :index="501">
        <v-a edit v-model="os" @save="oneditbanner" ref="edit"></v-a>
        <div class="h80"></div>
        <div class="w80">
          <van-button
            block
      ...
      <v-f :footer="false" v-model="open2" text="查看轮播图" :index="500">
        <v-d v-model="showext"></v-d>
      </v-f>
      <v-f :footer="false" v-model="open3" text="添加轮播图" :index="502">
        <v-a ref="add" @save="onaddbanner"></v-a>
        <div class="h80"></div>
        <div class="w80">
          <van-button
            block
