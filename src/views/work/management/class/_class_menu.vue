<template>
  <div class="class_menu">
    ...
    <template v-else-if="+type === 3">
      <v-a3 v-model="detail" :ids="ids"></v-a3>
    </template>
    <template v-else-if="+type === 4">
      <v-a4 v-model="detail" :ids="ids"></v-a4>
    </template>
    <template v-else-if="+type === 5">
      <v-a5 v-model="detail" :ids="ids" @del="$emit('del', ids)"></v-a5>
    </template>
...
export default {
  ...
  components: { vA1, vA2, vA3, vA4, vA5 },
  computed: {
    det: {
      get: function get() {
        return this.value;
  ...
  methods: {
    onagreebtn(n) {
      ...
      this.$api.http(api, { c_id: this.ids }, () => {
        this.$toast("操作成功");
        this.$emit("del", this.ids);
      });
    },
    ontel(v) {
    ...
    ontab() {
      this.appPath(
        "/management_classsplit/" + this.ids + "&" + this.type,
        true
      );
    },
    todet_val(e) {
    ...
    init() {
      this.$api.http("classstudentdet", { id: this.ids }, (r) => {
        this.detail = r;
