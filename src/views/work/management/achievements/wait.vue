<template>
  <v-view header bgc="#f5f5f5">
    ...
    <template #fixed>
      <v-f v-model="open" :footer="false" text="绩效审核">
...
export default {
  ...
  watch: {
    ...
    ids(n) {
      if (n.indexOf(",") > 0) {
        this.appPath(
          "/management_achievements_wait/" + this.type + "&" + n,
          true
        );
        this.idarr = n.split(",");
      }
    ...
    open(n) {
      if (!n) {
        this.appPath("/management_achievements_wait", true);
        this.ids = "";
        this.idarr = [];
      }
    },
  ...
  methods: {
    ...
    onhis() {
      ...
      this.appPath("/management_achievements_wait/" + n + "&0", true);
    },
    ondetail(e, j) {
      let id = [e.id, e.month_id, e.year_month, j];
      this.appPath(
        "/management_achievements_wait/" + this.type + "&" + id.join(","),
        true
      );
      this.idarr = id;
    },
