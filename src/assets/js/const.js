...
let wxshare = (opt = {}, type = "share", callback, hid = "0") => {
  ...
  console.log(hid);
  console.log(hid && hid === "1", "hid_1");
  console.log(hid, "hid");
  console.log(opt.link, window.location.href);
  if (!e.appId) return 0;
  demo.$vx.start(e, res => {
    if (res.code) {
      let obj = {
      ...
      try {
        ...
        if (
          obj.link.indexOf("/info/") >= 0 ||
          obj.link.indexOf("/article_detail") >= 0
        ) {
          console.log("分享链接改为家长端");
          obj.link = obj.link
            .replace("/yuansuo/", "/parent/")
            .replace("/article_detail", "/info_detail")
            .replace("/info", "/info_detail");
...
let wxshareh5 = (type, obj = {}, desc = "", flag, nametologo = {}) => {
  ...
  let url = [demo.siteConfig.api.h5, "poster/?type=", type, "&h5=", a].join("");
  console.log(url, env, nametologo.title, nametologo);
  if (flag) return url;
  if (env) {
    console.log(url);
    return 0;
  ...
  if (nametologo.title) {
    console.log("nametologo");
    wxshare(
      { link: url, desc, ...nametologo },
      "share",
      () => {},
      os.customize ? "1" : "0"
    );
  ...
  wxshare({ link: url, desc }, os.customize ? "1" : "0");
};
