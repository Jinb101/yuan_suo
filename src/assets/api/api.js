...
// 上传篇
let upload = (data, fn, error, progress) => {
  console.log(123,data, error);
  let url = link.loginApi.upload.api;
  let type = data.imgs.type.indexOf('image') < 0;
  console.log(data,type,data.imgs.type.indexOf('image'));
  if (type && data.type === 'img') {
    data.type = 'pdf';
  }
  if (data.imgs.type.indexOf('image') !== -1 && data.type === 'img') {
    Tools.compressFile(data.imgs).then((r)=>{
      data.imgs = r
    })
...
let post = (url, data, fn, fs) => {
  ...
  if (waitpostdata.indexOf(keymd5) >= 0) {
    waitpostcallback({ url, data, fs }, e => { fn(e) })
    return 0
  }
  if (data.page) {
...
let http = (name, data, fn, methods = 'post', isLogin = false, successCode = [200]) => {
  ...
  if (apiTime + st > ts && apiPath === name && JSON.stringify(data) === JSON.stringify(apiData)) {
    return 0
  }
  setTimeout(() => {
    if (loads) return 0
  ...
  if (method === 'post') {
    post(url.api, d, e => {
      ...
      if (e.code === 401) {
        ...
        route.push('/login')
        exit(true)
        let msg = e.msg.indexOf('未登录') > -1 ? '未登录，请登录' : '登录已过期，请重新登录'
      ...
      if (e.code === 409) {
        ...
        return model.alert(e.msg || e.err || '用户未注册或未绑定微信', () => { })
      }
      if (e.code === 444) {
...
export default {
  get, post, token, upload, exit, link, http, city, classes, isWwrkDot
}
